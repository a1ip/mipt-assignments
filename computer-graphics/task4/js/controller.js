function Controller(display, canvas) {
    ...
    canvas.addEventListener('mousedown', function(event) {
        ...
        _this.down = true;
        _this.mousedown(P(event.pageX, event.pageY), event.which);
    });
...
Controller.prototype.motion = function(mouse) {
    if (this.down) {
        if (this.display.selection != null)
...
Controller.prototype.mousedown = function(mouse, num) {
    if (num > 1) return;
    var h = this.canvas.height, w = this.canvas.width, d = this.display;
    if (d.selection == null) {
        if (d.chosen == null) {
            var newLom = PL();
            var newPoint = P(mouse.x / w, mouse.y / h, newLom);
            newLom.add(newPoint);
            d.polylines.push(newLom);
            d.add(newPoint);
        } else {
            var newPoint = P(mouse.x / w, mouse.y / h, d.chosen.parent);
            d.chosen.parent.add(newPoint);
            d.add(newPoint);
        }
    } else {
        d.choose(d.selection);
    }
    d.draw();
    return this;
};
...
Controller.prototype.press = function(key) {
    ...
    case 'a'.charCodeAt(0):
        ...
        break;
    case 'l'.charCodeAt(0):
        this.display.switchNorm();
        break;
    case 'r'.charCodeAt(0):
        this.display.changeRadius(0.5);
        break;
    case 'R'.charCodeAt(0):
        this.display.changeRadius(-0.5);
        break;
    case 'w'.charCodeAt(0):
        this.display.changeWeight(0.05, 0.0);
        break;
    case 'W'.charCodeAt(0):
        this.display.changeWeight(-0.05, 0.0);
        break;
    case 'q'.charCodeAt(0):
        this.display.changeWeight(0.0, 0.05);
        break;
    case 'Q'.charCodeAt(0):
        this.display.changeWeight(0.0, -0.05);
        break;
