...
DisplayGL2.prototype.draw = function() {
    ...
    gl.clear(this.gl.DEPTH_BUFFER_BIT);
    var bg_canvas = this.bg_canvas;
    var bg_context = this.bg_context;

    var dark = [], points = [], colors = [];
    this.points.forEach(function(p) {
        points.push(p.x);
        points.push(1 - p.y);
        var x = Math.floor(p.x * bg_canvas.width);
        var y = Math.floor(p.y * bg_canvas.height);
        var color = bg_context.getImageData(x, y, 1, 1).data;
