globals 
 [
   run-duration
 ;  initial-num-marketers  Moved to slider
...
to setup
   ...
   ; set initial-num-marketers 5  Moved to Interface
   set total-sales 0
   ...
   ask patches
    [
      set pcolor green
      set tolerance random 16 + 5   ; New for this version: tolerance is between 5 and 20
      set caller-memory-list []  ; initialize the memory list
    ]
...
to do-sales
   ...
   ; New for memory version
   set buyers buyers with [i-accept-call-from? myself]
   
   set weekly-sales count buyers
   
   set total-sales total-sales + weekly-sales
   
   ; ask buyers [set pcolor brown]  This is moved to i-accept-call-from?
...
to do-accounting
   ...
   set bank-balance bank-balance - weekly-cost + (weekly-sales * income-per-sale)
   
   ifelse bank-balance > growth-param
    [
      set size (size + (bank-balance - growth-param) * money-size-ratio)
      set bank-balance growth-param
    ]
    [
      if bank-balance < 0 [die]
    ]
    
end

to-report i-accept-call-from? [a-marketer]

  ; First see how many times the caller appears on the memory list
  ; Do this by getting length of a temporary list that contains only this caller
  let num-calls length filter [? = a-marketer] caller-memory-list
  
  ; write a-marketer  ; Use these test outputs only with very few turtles!
  ; show num-calls
  
  ; Then remember this call by adding it to the memory
  set caller-memory-list fput a-marketer caller-memory-list
  
  ; Set color brown to not answer more calls this tick
  set pcolor brown
  
  ; See if the tolerance was exceeded before this call
  ; If not, report "true" to accept the call
...
This version includes customer memory of which telemarketers have called, as described in Section 13.6. That change is not documented here.
...
<experiments>
  ...
  <experiment name="Growth parameter" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles</metric>
    <metric>total-sales</metric>
    <steppedValueSet variable="growth-param" first="200" step="200" last="3000"/>
  </experiment>
</experiments>
@#$#@#$#@
@#$#@#$#@
...
-- 
Вы получили это сообщение, поскольку подписаны на группу multiagent-2014.

Чтобы отменить подписку на эту группу и больше не получать от нее сообщения, отправьте письмо на электронный адрес multiagent-2014+unsubscribe@googlegroups.com.
Чтобы добавлять сообщения в эту группу, отправьте письмо по адресу multiagent-2014@googlegroups.com.
Просмотреть это обсуждение в Сети можно по адресу https://groups.google.com/d/msgid/multiagent-2014/5482423B.90501%40gmail.com.
