...
to setup
  ...
  ask patches
   ...
   set peak-activity-count -1
   
   set fly-distance 3
   set balls-per-trap 1
...
to pop
    ...
    ask n-of balls-per-trap patches in-radius fly-distance 
    [
...
to start
  ...
  while [any? patches with [trigger-time > ticks] ]
    ...
    [
      ...
      if count patches with [trigger-time > ticks] > peak-activity-count
        [ set peak-activity-time ticks
          set peak-activity-count count patches with [trigger-time > ticks]
        ]
    ]
    
    set last-trap-time ticks
    let triggered-traps count patches with [pcolor = black]
    let untriggered-traps count patches with [pcolor = yellow]
    set triggered-fraction 1.0 * triggered-traps / (triggered-traps + untriggered-traps)
    set stop-cond 1
end
@#$#@#$#@
GRAPHICS-WINDOW
...
<experiments>
  <experiment name="experiment" repetitions="1" runMetricsEveryStep="false">
    <setup>setup
start</setup>
    <metric>last-trap-time</metric>
    <metric>triggered-fraction</metric>
    <metric>peak-activity-time</metric>
    <steppedValueSet variable="fly-distance" first="3" step="1" last="7"/>
    <steppedValueSet variable="balls-per-trap" first="1" step="1" last="5"/>
  </experiment>
</experiments>
@#$#@#$#@
@#$#@#$#@
