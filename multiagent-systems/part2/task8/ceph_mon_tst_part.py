	...
	srv = random.choice(servers)
        try:
            ...
            with lock: 
                ...
                failed_writes += 1
                client_failed_writes[cli] = client_failed_writes.get(cli, 0) + 1
                failed_write_times.append((time.time(), failed_writes))
...
def plot_failed_write_times():
    times = map(lambda (x, y): x - failed_write_times[0][0], failed_write_times)
    writes = map(lambda (x, y): y, failed_write_times)
    plt.plot(times, writes)
    plt.xlabel('time')
    plt.ylabel('failed writes')
    plt.show()

if __name__=="__main__":
    test()
    for client, writes in client_failed_writes.items():
        print 'Client: #{0}, failed writes: {1}'.format(client, writes)
    plot_failed_write_times()
