...
to go
  ...
  ask turtles with [(age-in-months > 12) and (not is-alpha?)] [make-decision]
...
; decision is: if at least one bird are older than we are then scout
to make-decision
  let my-age age-in-months
  let counter 0
  let group turtles-here
  ifelse (is-female?)
  [ set group turtles-here with [is-female? and age-in-months > 12] ]
  [ set group turtles-here with [(not is-female?) and age-in-months > 12] ]
  ask group [
    if (age-in-months > my-age) [set counter counter + 1]
  ]
  if (counter >= 1) [ scout ]
end
